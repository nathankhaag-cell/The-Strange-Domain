...
function showEmojiPicker(msgId, btn) {
    ...
    // Mobile: CSS pins to bottom of screen — no JS positioning needed
    ...
    // Desktop: measure actual rendered width first, then position
    ...
    // Center under the button, clamped so it never bleeds off screen
    let left = rect.left + (rect.width / 2) - (pickerW / 2);
    left = Math.max(margin, Math.min(left, vw - pickerW - margin));

    // Prefer below button, flip above if not enough room below
